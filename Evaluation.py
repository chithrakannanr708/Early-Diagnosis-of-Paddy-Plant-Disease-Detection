...
def Table_evaluation(sp, act):
    ...
    prevalence = ((tp + fn) / (tp + fp + tn)) * 100
    TS = (tp / (tp + fn + fp)) * 100
    EVAL = [tp, tn, fp, fn, accuracy, For, ba, fm, bm, sensitivity, F1_score, MCC, specificity, precision, FPR, FNR, NPV,
            FDR, pt,
            mk, PLHR, lrminus, dor2, prevalence, TS]
    return EVAL
